  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const approvedPropertyDetailsPromises = approvedRequests.map(
          (request) =>
            axios
              .get(
        ...
        const appProperties = await Promise.all(
          approvedPropertyDetailsPromises
        );
  ...
  const handleDeleteRequest = async (requestId) => {
    try {
      ...
      setPendingProperties((prevProperties) =>
        prevProperties.filter((property) => property._id !== requestId)
      );
      setApprovedProperties((prevProperties) =>
        prevProperties.filter((property) => property._id !== requestId)
      );
    } catch (error) {
      ...
      <div className="tenant-request-list">
        {pendingProperties.map((request) => (
          <div key={request._id} className="tenant-request-card">
            <h2>
              Property: {request.property ? request.property.place : "N/A"}
            </h2>
            <p>
              Area: {request.property ? request.property.area : "N/A"} sq ft
            </p>
            <p>
              Bedrooms: {request.property ? request.property.bedrooms : "N/A"}
            </p>
            <p>
              Bathrooms: {request.property ? request.property.bathrooms : "N/A"}
            </p>
            <p>
              Nearby Hospitals:{" "}
              {request.property ? request.property.hospitals : "N/A"}
            </p>
            <p>
              Nearby Colleges:{" "}
              {request.property ? request.property.colleges : "N/A"}
            </p>
